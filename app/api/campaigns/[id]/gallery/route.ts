...
function galleryResponse(access: any) {
  ...
  return { images, folders: getGalleryFolders(access.campaign, state.galleryFolders), requests }
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === "replace") {
    ...
    images = body.images.map((image: unknown) => sanitizeGalleryImage(image)).filter((image: GalleryImage | null): image is GalleryImage => Boolean(image)).map((image: GalleryImage) => ({
      ...image,
      folderId: folderIds.has(image.folderId || "root") ? image.folderId : "root",
    }))
  } else if (action === "add") {
    ...
    if (resolution === "approved") publish(campaignId, { type: "gallery:image-show", url: galleryRequest.imageUrl, name: galleryRequest.imageName })
    return NextResponse.json(galleryResponse(access))
  ...
  updateCampaignState(campaignId, {
    gallery: images,
    galleryFolders: folders.filter((folder) => !folder.isPlayerFolder),
    galleryBroadcastRequests: requests,
  })
