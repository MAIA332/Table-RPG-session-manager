...
import { CustomItemWeightEditor } from "./item-weight-field"
...
function rollDicePool(dieSizes: number[], minimum?: number, modifier = 0) {
  if (minimum === undefined) {
    ...
    return {
      details,
      total: details.reduce((sum, roll) => sum + roll, 0),
      modifier,
    }
  }
  ...
  if (eligibleRolls.length > 0) {
    const details =
      eligibleRolls[Math.floor(Math.random() * eligibleRolls.length)]
    return {
      details,
      total: details.reduce((sum, roll) => sum + roll, 0),
      modifier,
...
export function ItemModifiers({ text }: { text: string }) {
  ...
  const descMatch = text.split(
    /\[(?:BÔNUS|BONUS|MODIFICADOR|LORE|PENALIDADE|UTILIDADE)\s*:/i,
  )[0]
        ...
        <div className="flex flex-col gap-1.5 mt-2 w-full">
          ...
          {lore && lore.toLowerCase() !== "nenhum" && (
            <div className="inline-flex items-center gap-1.5 bg-purple-500/15 text-purple-400 border border-purple-500/40 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest shadow-[0_0_10px_rgba(168,85,247,0.1)] w-fit">
              ...
              <div className="text-left">{lore}</div>
            </div>
          )}
          {utilidade && utilidade.toLowerCase() !== "nenhum" && (
            <div className="inline-flex items-center gap-1.5 bg-blue-500/15 text-blue-400 border border-blue-500/40 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest shadow-[0_0_10px_rgba(59,130,246,0.1)] w-fit">
              ...
              <div className="text-left">{utilidade}</div>
            </div>
          )}
        </div>
      )}
...
function CombinedChecksPanel({
  ...
  const filtered = PRESET_CHECKS.filter(
    (c) =>
  ...
  return (
    <div className="flex flex-col gap-4 mt-2 animate-in fade-in zoom-in-95 duration-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[380px] overflow-y-auto custom-scrollbar-sepia pr-2">
        {filtered.map((check) => (
          ...
          >
            <div className="flex flex-col items-start w-full gap-1">
              <div className="flex justify-between items-center w-full">
                ...
                <div className="flex gap-1.5">
                  {check.attrs.map((attr: any, i: number) => (
                    <span
                      key={i}
                      className="text-[10px] bg-primary/20 text-primary border border-primary/30 px-1.5 py-0.5 rounded font-mono uppercase font-bold"
                    >
                      {attr}{" "}
              ...
              <div className="text-xs font-normal opacity-60 text-left whitespace-normal leading-snug">
                {check.desc}
              </div>
            </div>
          </Button>
        ...
        {filtered.length === 0 && (
          <div className="text-sm text-muted-foreground italic text-center col-span-2 py-8">
            Nenhum teste encontrado com esse nome.
          </div>
        )}
      </div>
    </div>
...
function BlueprintApp({ character, editable, onSpendMp }: any) {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 bg-destructive/10 border border-destructive/30 rounded-xl text-center">
        ...
        <p className="text-sm text-destructive/80 mt-2 max-w-sm">
          Este Almanaque possui travas de segurança intrincadas. Apenas um
          Inventor treinado com a perícia <strong>"Aparelhos"</strong> pode
          decifrar e construir estes projetos Magitech.
        </p>
      </div>
    )
  }
  ...
  const blueprints = [
    {
      level: 1,
      name: "Magiesfera Luminosa",
      mpCost: 5,
      ipCost: 1,
      reqItem: null,
      desc: "Cria uma esfera de luz. Requer 1 IP como peça.",
    },
    {
      level: 2,
      name: "Gancho Pneumático",
      mpCost: 10,
      ipCost: 0,
      reqItem: "sucata",
      reqItemName: "Sucata de Ferro",
      desc: "Permite escalar superfícies. Consome uma [Sucata de Ferro].",
    },
    {
      level: 3,
      ...
      ipCost: 2,
      reqItem: null,
      desc: "Cobre a área em fumaça. Requer 2 IP de reagentes.",
    },
    {
      level: 4,
      name: "Drone Escoteiro",
      mpCost: 20,
      ipCost: 0,
      reqItem: "lente-magica",
      reqItemName: "Lente Mágica",
      desc: "Pequeno robô voador. Consome uma [Lente Mágica].",
    },
    {
      level: 5,
      name: "Canhão Magitech",
      mpCost: 30,
      ipCost: 0,
      reqItem: "nucleo-energia",
      reqItemName: "Núcleo de Energia",
      desc: "Dispara uma rajada concentrada. Consome um [Núcleo de Energia].",
    },
  ]
  ...
  function handleCraft(bp: any) {
    ...
    if (currentMp < bp.mpCost)
      return alert(`Sua Mente (MP) é insuficiente. Necessário: ${bp.mpCost}.`)

    if (bp.ipCost > 0) {
      return alert(
        "Este projeto ainda usa IP como material. Cadastre os materiais como itens com peso antes de utilizá-lo.",
      )
    } else if (bp.reqItem) {
      const itemIndex = character.equipment.findIndex((itemId: string) =>
        itemId.includes(bp.reqItem),
      )
      if (itemIndex === -1)
        return alert(
          `Falta material: Você precisa ter o item [${bp.reqItemName}] no equipamento para construir isto.`,
        )
      onSpendMp(bp.mpCost)
    ...
    <div className="flex flex-col gap-3">
      <div className="bg-primary/10 border border-primary/30 p-4 rounded-xl mb-2 shadow-inner">
        <p className="text-sm text-primary font-bold">
          Nível da Perícia 'Aparelhos': {skillLvl}
        </p>
        <p className="text-xs text-muted-foreground mt-1">
          Sua mochila possui:{" "}
          <span className="font-bold text-white">{currentIp} IP</span>.
        </p>
      </div>
      ...
      {blueprints.map((bp) => {
        ...
        let hasItem = false
        if (bp.reqItem) {
          hasItem = character.equipment.some((itemId: string) =>
            itemId.includes(bp.reqItem!),
          )
        }
        ...
        const canAfford =
          currentMp >= bp.mpCost &&
          (bp.ipCost > 0 ? currentIp >= bp.ipCost : hasItem)

        return (
          <div
            key={bp.level}
            className={`p-4 rounded-xl border transition-all ${unlocked ? "border-border/50 bg-card/50" : "border-destructive/20 bg-destructive/5 opacity-60 grayscale"}`}
          >
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
              <div className="flex-1 pr-4">
                <h4
                  className={`font-bold ${unlocked ? "text-foreground" : "text-destructive"} flex items-center gap-2 flex-wrap`}
                >
                  {bp.name}
                  <span className="text-[10px] bg-background border border-border px-1.5 py-0.5 rounded text-muted-foreground uppercase tracking-widest">
                    Req: Nv. {bp.level}
                  </span>
                </h4>
              ...
              {unlocked ? (
                <Button
                  ...
                  className={`gap-2 shrink-0 font-bold h-9 ${canAfford ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-zinc-800 text-zinc-500"}`}
                >
                  <Zap className="size-3.5" />{" "}
                  {canAfford ? "Construir" : "Materiais Insuf."}
                </Button>
              ) : (
                <span className="text-[10px] uppercase font-bold text-destructive flex items-center gap-1.5 shrink-0 bg-background/50 px-2 py-1 rounded">
...
export function NPCCreator({
  ...
  const [customAttributes, setCustomAttributes] = useState<
    Record<AttributeKey, DieSize>
  >({ dex: "d8", ins: "d8", mig: "d8", wlp: "d8" })
  const [equipment, setEquipment] = useState<string[]>([])

  const totalLevels = useMemo(
    () => Object.values(skillLevels).reduce((s, n) => s + n, 0),
    [skillLevels],
  )
  ...
  const customPoints = useMemo(
    () =>
      Object.values(customAttributes).reduce((acc, die) => {
        return (
          acc + (die === "d6" ? 1 : die === "d8" ? 2 : die === "d10" ? 3 : 4)
        )
      }, 0),
    [customAttributes],
  )
  ...
  const classesPayload: ClassLevel[] = useMemo(
    () =>
      Object.entries(classLevels).map(([classId, level]) => ({
        classId,
        level,
      })),
    [classLevels],
    ...
    <div className="mx-auto max-w-3xl w-full">
      <div className="mb-8 flex items-center justify-between">
        {NPC_STEPS.map((label, i) => (
          <div key={label} className="flex flex-1 items-center">
            <div
              className={`flex size-9 shrink-0 items-center justify-center rounded-full border text-sm font-bold transition-colors ${i < step ? "border-destructive bg-destructive text-destructive-foreground" : i === step ? "border-destructive bg-destructive/15 text-destructive" : "border-border text-muted-foreground"}`}
            >
              {i < step ? <Check className="size-4" /> : i + 1}
            </div>
            <span
              className={`ml-2 hidden text-sm sm:inline ${i === step ? "font-semibold text-foreground" : "text-muted-foreground"}`}
            >
              {label}
            </span>
            {i < NPC_STEPS.length - 1 && (
              <div
                className={`mx-2 h-px flex-1 ${i < step ? "bg-destructive" : "bg-border"}`}
              />
            )}
          </div>
        ))}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 rounded-lg border border-destructive/40 bg-destructive/5 px-4 py-3 text-sm">
        ...
        <span className="flex items-center gap-1.5 text-muted-foreground">
          <Swords className="size-4" /> Lv. {Math.max(1, totalLevels)}
        </span>
      </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        {step < NPC_STEPS.length - 1 ? (
          ...
          <Button
            ...
            className="gap-1.5 font-semibold bg-destructive hover:bg-destructive/90 text-destructive-foreground"
          >
            {saving ? (
...
export function CreatureSheet({
  ...
  onRoll: (attr: string, res: number) => void
  onKill?: () => void
  onUseAttack?: (index?: number, abilityId?: string) => void
  onOpenCombat?: () => void
}) {
  ...
  function useInventoryItem(actionId: string) {
    ...
    if (act.effectResource === "hp")
      updates.currentHp = Math.min(
        creature.maxHp,
        currentHp + (act.effectValue || 0),
      )
    if (act.effectResource === "mp")
      updates.currentMp = Math.min(
        creature.maxMp,
        currentMp + (act.effectValue || 0),
      )
  ...
  const affinitiesEntries = Object.entries(creature.affinities || {}) as [
    string,
    string,
  ][]
    ...
    <div className="panel border-glow relative rounded-xl border border-destructive/50 p-5 flex flex-col w-full h-full bg-zinc-950 shadow-[0_0_30px_rgba(255,0,0,0.1)]">
      <AnimatePresence>
        {showInventory && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 overflow-hidden"
            onClick={() => setShowInventory(false)}
          >
            <motion.div
              variants={modalVariants}
              className="rpg-modal relative flex h-full max-h-[85vh] w-full max-w-4xl flex-col overflow-hidden border border-destructive/50 bg-zinc-950 shadow-2xl"
              onClick={(event) => event.stopPropagation()}
            >
              <div className="flex justify-between items-center p-6 border-b border-white/10 bg-black/40 shrink-0">
                <h4 className="font-serif text-2xl font-black text-destructive flex items-center gap-3">
                  <Package className="size-6" /> Saque da Criatura
                </h4>
                <button
                  onClick={() => setShowInventory(false)}
              ...
              <div className="flex-1 overflow-y-auto p-6 flex flex-col md:flex-row gap-10 custom-scrollbar-sepia">
                <section className="flex-1 space-y-4">
                  <h5 className="text-sm font-bold uppercase tracking-widest text-muted-foreground border-b border-white/10 pb-2">
                    Equipamento da Criatura
                  </h5>
                  {equipment.length === 0 ? (
                    <p className="p-6 text-center rounded-lg border border-dashed border-border/40 text-sm text-muted-foreground italic">
                      Nenhum equipamento.
                    </p>
                  ) : (
                    <div className="flex flex-col gap-3">
                      {equipment.map((id: string, idx: number) => {
                        const item =
                          customEquipment.find((e: any) => e.id === id) ||
                          getEquipment(id)
                        return item ? (
                          <div
                            key={`${id}-${idx}`}
                            className="flex items-start gap-3 p-3 rounded-lg bg-card border border-destructive/20"
                          >
                ...
                <section className="flex-1 space-y-4">
                  <div className="flex justify-between items-center border-b border-white/10 pb-2">
                    ...
                    <span className="text-sm font-mono font-bold text-destructive bg-destructive/10 px-2 py-1 rounded">
                      IP: {currentIp}/6
                    </span>
                  </div>
                  {isGm ? (
                    <div className="flex flex-col gap-3">
                      {INVENTORY_ACTIONS.map((act: any) => (
                        <Button
                          key={act.id}
                          variant="secondary"
                          className="h-auto py-3 px-4 justify-between items-center group border border-border/50 hover:border-destructive/50"
                          onClick={() => useInventoryItem(act.id)}
                        >
                          <div className="text-left flex flex-col gap-0.5">
                            <span className="font-bold text-foreground group-hover:text-destructive transition-colors">
                              {act.name}
                            </span>
                          ...
                          <span className="font-mono text-sm font-bold text-destructive shrink-0 ml-4 bg-background px-2 py-1 rounded">
                            -{act.cost} IP
                          </span>
                        </Button>
                      ))}
                    ...
                    <p className="p-6 text-center text-sm text-muted-foreground">
                      Apenas o mestre manipula inventário de monstros.
                    </p>
                  )}
                </section>
              </div>
        ...
        {isGm && (
          <div className="flex flex-col items-end shrink-0">
            <Button
              variant="outline"
              size="sm"
              className="h-auto py-2 border-destructive/30 text-destructive hover:bg-destructive/20 shrink-0"
          ...
          <div className="mt-6 grid grid-cols-4 gap-2">
            {ATTR_KEYS.map((k: AttributeKey) => {
              ...
              return (
                <button
                  key={k}
                  disabled={!isGm || isRolling}
                  onClick={() => rollDice(k, creature.attributes[k])}
                  className={`attribute-roll-button rounded-lg border py-3 text-center ${isRolling ? "is-rolling border-destructive bg-destructive/20" : "border-border/60 bg-card/40 hover:border-destructive/50 hover:bg-card"}`}
                >
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-1">
                    {k}
                  </p>
                  <p className="font-serif text-xl font-black text-destructive drop-shadow-sm">
                    {creature.attributes[k]}
                  </p>
                </button>
              )
            })}
      ...
      {isGm ? (
        <div className="mt-6 flex flex-col gap-4">
          <ResourceBar
            ...
            onChange={(d) =>
              patchVital(
                "currentHp",
                Math.max(0, Math.min(creature.maxHp, currentHp + d)),
              )
            }
          />
          <ResourceBar
            ...
            onChange={(d) =>
              patchVital(
                "currentMp",
                Math.max(0, Math.min(creature.maxMp, currentMp + d)),
              )
            }
          />
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4">
          <CreatureVitals current={currentHp} max={creature.maxHp} />
          <CreatureVitals current={currentMp} max={creature.maxMp} mana />
        </div>
      )}
      ...
      {isGm && onOpenCombat && (
        <button
          ...
          className="mt-4 rounded-lg border border-amber-300/40 p-3 font-bold text-amber-200"
        >
          Abrir habilidades e QTEs no combate
        </button>
      )}
        ...
        {creature.basicAttacks && creature.basicAttacks.length > 0 && (
          <div>
            ...
            <div className="flex flex-col gap-2">
              {(creature.basicAttacksV2 || creature.basicAttacks).map(
                (atk: any, i: number) => (
                  <div
                    key={i}
                    className="bg-card/40 border border-border/30 p-2 rounded text-sm"
                  >
                    {isGm && onUseAttack && (
                      <button
                        type="button"
                        onClick={() => onUseAttack(i)}
                        className="mb-2 rounded border border-red-400/40 bg-red-950 px-3 py-2 text-sm font-bold text-red-100"
                      >
                        Usar ataque · selecionar alvo
                      </button>
                    )}
                    <p className="font-bold text-destructive">
                      {atk.name}{" "}
                      <span className="font-mono text-xs text-muted-foreground ml-2">
                        [{atk.attributes.join(" + ").toUpperCase()}]
                      </span>
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      Dano:{" "}
                      ...
                      ({atk.type}) {atk.description && `- ${atk.description}`}
                    </p>
                  </div>
                ),
              )}
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              {creature.spells.map((spell: string, i: number) => (
                <div
                  key={i}
                  className="bg-purple-900/10 border border-purple-500/20 p-2 rounded text-sm"
                >
...
function CharacterStatusBadges({ modifiers }: { modifiers: Modifier[] }) {
  ...
  const getVisual = (modifier: Modifier) => {
    ...
    if (name.includes("queim") || name.includes("fogo"))
      return { Icon: Flame, tone: "burn" }
    if (name.includes("sangr")) return { Icon: Droplets, tone: "bleed" }
    if (name.includes("fraq") || name.includes("vulner"))
      return { Icon: ShieldOff, tone: "weak" }
    if (name.includes("atordo") || name.includes("confus"))
      return { Icon: Brain, tone: "stun" }
    if (name.includes("medo") || name.includes("maldi"))
      return { Icon: Ghost, tone: "curse" }
    if (name.includes("venen") || name.includes("tox"))
      return { Icon: Skull, tone: "poison" }
    return {
      Icon: Activity,
      tone: modifier.value < 0 ? "negative" : "positive",
    }
  }
  ...
  return (
    <div className="character-status-list" aria-label="Condições ativas">
      {modifiers.slice(0, 3).map((modifier) => {
        ...
        return (
          <span
            key={modifier.id}
            data-tone={tone}
            className="character-status-icon"
            title={detail}
            aria-label={detail}
          >
      ...
      {modifiers.length > 3 && (
        <span
          className="character-status-more"
          title={modifiers
            .slice(3)
...
const isFreeSkillRecord = (mod: any) =>
  typeof mod.id === "string" && mod.id.startsWith(FREE_SKILL_PREFIX)
const skillRank = (value: unknown): number => {
  const rank = Number(value)
  return Number.isFinite(rank) ? Math.max(0, Math.floor(rank)) : 0
}
const skillMaxRank = (skill: any): number =>
  Math.max(1, skillRank(skill.maxLevel ?? 1))

function getFreeSkillRanks(
  skillId: string,
  level: number,
  modifiers: any[],
  legacyCustomIds: Set<string>,
): number {
  const record = modifiers.find(
    (mod) => mod.id === `${FREE_SKILL_PREFIX}${skillId}`,
  )
  // Compatibilidade: habilidades personalizadas antigas já eram gratuitas.
  return Math.min(
    level,
    record ? skillRank(record.value) : legacyCustomIds.has(skillId) ? level : 0,
  )
}
...
function getSpentSkillPoints(
  ...
  for (const owned of classes) {
    ...
    const classSkills = (definition?.skills ?? []).filter(
      (skill: any) => !counted.has(skill.id),
    )
    let ranks = 0
    let free = 0
    ...
    const isLegacyCustom =
      ...
      classSkills.every((skill: any) => customIds.has(skill.id))
...
const MANUAL_DEFENSE_IDS = {
  defense: "sheet_defense:physical",
  magicDefense: "sheet_defense:magical",
} as const
const isManualDefenseRecord = (mod: any) =>
  Object.values(MANUAL_DEFENSE_IDS).some((id) => mod.id === id)
...
function buildManualDefenseUpdates(
  modifiers: any[],
  defense: number,
  magicDefense: number,
) {
  if (
    ![defense, magicDefense].every(
  ...
  return {
    customModifiers: [
      ...
      {
        id: MANUAL_DEFENSE_IDS.defense,
        name: "Defesa manual",
        target: "sheet_defense_hidden",
        value: defense,
      },
      {
        id: MANUAL_DEFENSE_IDS.magicDefense,
        name: "Defesa mágica manual",
        target: "sheet_defense_hidden",
        value: magicDefense,
      },
    ],
...
function ManualDefenseFields({
  ...
  const savedMagicDefense = readManualDefense(
    modifiers,
    MANUAL_DEFENSE_IDS.magicDefense,
  )
  const initialDefense =
    savedDefense ?? defenseBaseValue(character.attributes.dex)
  const initialMagicDefense =
    savedMagicDefense ?? defenseBaseValue(character.attributes.ins)
  const [draft, setDraft] = useState({
    defense: String(initialDefense),
    magicDefense: String(initialMagicDefense),
  })
  ...
  const needsSave =
    dirty || savedDefense === undefined || savedMagicDefense === undefined
  async function save() {
    ...
    setError(null)
    try {
      const updates = buildManualDefenseUpdates(
        modifiers,
        defense,
        magicDefense,
      )
      ...
      if (
        readManualDefense(
          updated.customModifiers || [],
          MANUAL_DEFENSE_IDS.defense,
        ) !== defense ||
        readManualDefense(
          updated.customModifiers || [],
          MANUAL_DEFENSE_IDS.magicDefense,
        ...
        throw new Error(
          "A API não confirmou as defesas. Confira se a rota de personagens preserva customModifiers.",
        )
      }
      setDirty(false)
    } catch (err) {
  ...
  return (
    <section className="mt-5 rounded-xl border border-primary/25 bg-card/40 p-4">
      <div className="mb-3 flex items-center gap-2 text-sm font-bold text-primary">
        <Shield className="size-4" /> Defesas
      </div>
      <div className="grid grid-cols-2 gap-3">
        {(
          ...
          <label
            key={field.key}
            className="flex min-w-0 flex-col gap-2 rounded-lg border border-border/50 bg-background/50 p-3"
          >
            ...
            <span className="text-[10px] text-muted-foreground">
              Valor inicial: dado de {field.base}
            </span>
          </label>
        ))}
      </div>
      <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
        Ajuste manualmente conforme seus itens e habilidades. Os valores salvos
...
export function CharacterSheet({
  ...
  const character = useMemo(
    () => applyInventoryRules(rawCharacter, customEquipment),
    [rawCharacter, customEquipment],
  )
  ...
  const bonds: Bond[] = [
    ...bondsFromMods,
    ...oldBonds.filter((ob) => !bondsFromMods.some((bm) => bm.id === ob.id)),
  ]

  const displayModifiers = allMods.filter(
    (m: any) =>
      m.type !== "bond" &&
      !isFreeSkillRecord(m) &&
      !isItemState(m) &&
      !isManualDefenseRecord(m),
  )
  // --- FIM SOLUÇÃO PERSISTENCIA LAÇOS ---

  const [mounted, setMounted] = useState(false)
  const [pending, setPending] = useState(false)
  const [displayResources, setDisplayResources] = useState<CharacterResources>(
    character.resources,
  )
  ...
  const [sheetTab, setSheetTab] = useState<"main" | "checks" | "modifiers">(
    "main",
  )
  ...
  const [bondDraft, setBondDraft] = useState<Partial<Bond>>({
    target: "",
    type: "Amizade",
    value: 1,
  })
  ...
  useLayoutEffect(() => {
    ...
    const scroller = sheetRef.current.closest(
      ".rpg-character-scroll",
    ) as HTMLElement | null
    if (!scroller || scroller.scrollTop <= 0) return

    const compactReference = Array.from(
      scroller.querySelectorAll<HTMLElement>(
        ".rpg-character-sheet[data-expanded='false']",
      ),
    ).find((element) => element !== sheetRef.current)
    const compactHeight =
      compactReference?.getBoundingClientRect().height ?? 108
    const collapseDistance = Math.max(
      0,
      sheetRef.current.getBoundingClientRect().height - compactHeight,
    )
    const target = Math.min(
      scroller.scrollTop,
      Math.max(
        0,
        scroller.scrollHeight - scroller.clientHeight - collapseDistance,
      ),
    )
  ...
  useEffect(() => {
    if (resourceWriteCountRef.current > 0) {
      resourceCharacterRef.current = {
        ...character,
        resources: resourceDraftRef.current,
      }
      return
    }
  ...
  const itemCatalog = [
    ...customEquipment,
    ...EQUIPMENT.filter(
      (base) => !customEquipment.some((item: any) => item.id === base.id),
    ),
  ]
  ...
  const weightStatus = inventoryStatus(
    { ...character, resources: displayResources },
    itemCatalog,
  )
  const inventoryCapacity = Math.max(0, Number(displayResources.maxIp) || 0)
  const inventoryOverflow = Math.max(
    0,
    weightStatus.grams - weightStatus.carryLimitGrams,
  )
  const transferTargets = (campaignCharacters as Character[])
    .filter(
      (target) =>
        target.id !== character.id && target.ownerId !== character.ownerId,
    )
    .map((target) => ({
      id: target.id,
      name: target.name,
      ownerName: campaignMembers.find(
        (member: Member) => member.userId === target.ownerId,
      )?.name,
    }))
  ...
  const customSkillIds = new Set<string>(
    (customClasses || []).flatMap((c: any) =>
      (c.skills || []).map((skill: any) => skill.id),
    ),
  )
  const totalSkillPointsSpent = getSpentSkillPoints(
    skillsObj,
    character.classes || [],
    allClasses,
    allMods,
  ...
  const learnedSkillGroups = allClasses
    .map((definition: any) => {
      const learnedSkills = definition.skills.filter(
        (skill: any) => skillRank(skillsObj[skill.id]) > 0,
      )
      const owned = (character.classes || []).find(
        (c: any) => (c.classId ?? c.id) === definition.id,
      )
      return {
        ...definition,
        learnedSkills,
        level: Math.max(
          skillRank(owned?.level),
          learnedSkills.reduce(
            (sum: number, skill: any) => sum + skillRank(skillsObj[skill.id]),
            0,
          ),
        ),
        freeRanks: learnedSkills.reduce(
          (sum: number, skill: any) =>
            sum +
            getFreeSkillRanks(
              skill.id,
              skillRank(skillsObj[skill.id]),
              allMods,
              customSkillIds,
            ),
          0,
  ...
  const unresolvedSkills = Object.entries(skillsObj).filter(
    ([id, level]) =>
      skillRank(level) > 0 &&
      !allClasses.some((c) => c.skills.some((skill: any) => skill.id === id)),
  )

  const currentAttrPoints = Object.values(character.attributes).reduce(
    (acc: number, die: any) => {
  ...
  async function handleUpgradeAttribute(attrKey: AttributeKey) {
    ...
    const oldMaxes = computeMaxResources(
      character.classes || [],
  ...
  async function commitItemUpdate(updates: any) {
    if (
      (!editable && !isGm) ||
      pending ||
      itemCommitRef.current ||
      resourceWriteCountRef.current > 0
    ) {
    ...
    assertInventoryChange(
      character,
      {
        ...character,
        ...updates,
        resources: { ...character.resources, ...updates.resources },
      },
      itemCatalog,
    )
  ...
  function patchResource(key: string, delta: number) {
    if (key === "ip") {
      alert("IP representa kg livres e é recalculado pelo peso dos itens.")
      return
    }
    ...
    const request = resourceWriteQueueRef.current
      ...
      .then(async () => {
        try {
          const { character: updated } = await apiFetch<{
            character: Character
          }>(`/api/characters/${character.id}`, {
            method: "PATCH",
            body: JSON.stringify({ resources: { [key]: next[key] } }),
          })
          resourceConfirmedRef.current = updated.resources
          resourceWriteCountRef.current -= 1
          if (resourceWriteCountRef.current === 0) {
  ...
  async function saveZenit(value: number) {
    ...
    try {
      const { character: updated } = await apiFetch<{ character: Character }>(
        `/api/characters/${character.id}`,
        { method: "PATCH", body: JSON.stringify({ zenit: nextZenit }) },
      )
      onOptimistic(updated)
    } finally {
      setPending(false)
  ...
  async function savePortraitFrame(frame: PortraitFrameId) {
    if (
      !editable ||
      pending ||
      frame === (character.portraitFrame || "bronze")
    ) {
    ...
    try {
      const { character: updated } = await apiFetch<{ character: Character }>(
        `/api/characters/${character.id}`,
        { method: "PATCH", body: JSON.stringify({ portraitFrame: frame }) },
      )
      onOptimistic(updated)
    } finally {
      setPending(false)
  ...
  async function savePortraitImage(
    avatarUrl: string,
  ...
  async function saveNewSkillPoint(skillId: string) {
    if (
      (!editable && !isGm) ||
      pending ||
      skillWriteRef.current ||
      resourceWriteCountRef.current > 0
    )
      return
    const classMatch = allClasses.find((c) =>
      c.skills.some((skill: any) => skill.id === skillId),
    )
    ...
    const existingClass = newClasses.find(
      (c: any) => (c.classId ?? c.id) === classMatch.id,
    )
    const previousRanks = classMatch.skills.reduce(
      (sum: number, entry: any) => sum + skillRank(skillsObj[entry.id]),
      0,
    )
    if (existingClass)
      existingClass.level =
        Math.max(skillRank(existingClass.level), previousRanks) + 1
    else newClasses.push({ classId: classMatch.id, level: previousRanks + 1 })

    const freeRanks =
      getFreeSkillRanks(skillId, currentRank, allMods, customSkillIds) +
      (isGm ? 1 : 0)
    ...
    const newMods = [
      ...
      {
        id: grantId,
        name: `Concessão gratuita: ${skill.name}`,
        target: "skill_grant_hidden",
        value: freeRanks,
      },
    ]
    ...
    try {
      if (!isGm) {
        ...
        const baseClasses = (classes: any[]) =>
          classes.filter((c) =>
            CLASSES.some((base) => base.id === (c.classId ?? c.id)),
          )
        const oldMaxes = computeMaxResources(
          baseClasses(character.classes || []),
          character.attributes,
        )
        const newMaxes = computeMaxResources(
          baseClasses(newClasses),
          character.attributes,
        )
        updates.resources = {
          maxHp:
            (displayResources.maxHp ?? oldMaxes.maxHp) +
            newMaxes.maxHp -
            oldMaxes.maxHp,
          maxMp:
            (displayResources.maxMp ?? oldMaxes.maxMp) +
            newMaxes.maxMp -
            oldMaxes.maxMp,
          maxIp:
            (displayResources.maxIp ?? oldMaxes.maxIp) +
            newMaxes.maxIp -
            oldMaxes.maxIp,
        }
      }
      ...
      if (
        skillRank(saved.skills?.[skillId]) !== newSkills[skillId] ||
        !(saved.classes || []).some(
          (c: any) =>
            (c.classId ?? c.id) === classMatch.id &&
            skillRank(c.level) ===
              newClasses.find((c: any) => (c.classId ?? c.id) === classMatch.id)
                .level,
        ) ||
        !(saved.customModifiers || []).some(
          (mod: any) =>
            mod.id === grantId && skillRank(mod.value) === freeRanks,
        )
      ) {
        setSkillSaveError(
          "A API não confirmou todos os dados da concessão. Recarregue a ficha e revise a rota PATCH de personagens antes de tentar novamente.",
        )
        return
      }
      ...
      setSkillSaveError(
        error instanceof Error
          ? error.message
          : "Não foi possível salvar a habilidade. Tente novamente.",
      )
    } finally {
  ...
  async function updateBonds(newBonds: Bond[]) {
    ...
    const baseMods = ((character as any).customModifiers || []).filter(
      (m: any) => m.type !== "bond",
    )
  ...
  async function buyItem(itemId: string) {
    if (!editable || pending) return
    const item = itemCatalog.find((e) => e.id === itemId)
    if (!item) return
    ...
    try {
      await commitItemUpdate({
        equipment: [...(character.equipment || []), item.id],
        zenit: currentZenit - item.cost,
      })
    } catch (error) {
      alert(
        error instanceof Error
  ...
  async function sellItem(itemId: string, index: number) {
    if (!editable) return
    const item =
      customEquipment.find((e: any) => e.id === itemId) || getEquipment(itemId)
    if (!item) return
    const sellValue = Math.floor(item.cost / 2)

    const currentEquip = Array.isArray(character.equipment)
      ? character.equipment
      : []
    ...
    try {
      const { character: updated } = await apiFetch<{ character: Character }>(
        ...
        {
          method: "PATCH",
          body: JSON.stringify({ equipment: newEquipment, zenit: newZenit }),
        },
      )
      onOptimistic(updated)
    } finally {
  ...
  async function removeEquipmentItem(index: number) {
    ...
    if (
      !confirm(
        "Tem certeza que deseja remover este item da mochila do jogador?",
      )
    )
      return
  ...
  function rollDice(attr: AttributeKey, dieString: string) {
    ...
    setTimeout(() => {
      ...
      let modTotal = sumItemBonus(activeItemBonuses, [
        attr,
      ...
      if (onRoll)
        onRoll(
          detailStr,
          finalResult,
          modTotal !== 0 ? { modifier: modTotal } : undefined,
        )

      if (activeBond) setActiveBond(null) // Consome o laço invocado
      window.setTimeout(
        () => setRollingAttr((current) => (current === attr ? null : current)),
        4300,
      )
    }, 800)
  }
  ...
  function handleCombinedRoll(check: any) {
    ...
    setTimeout(() => {
      ...
      if (onRoll)
        onRoll(logDetail, finalResult, {
          breakdown: details.join(" + "),
          modifier: modTotal || undefined,
        })
  ...
  function useSkill(skill: any) {
    ...
    if (skill.action.resource === "ip") {
      alert(
        "Esta habilidade usa a regra antiga de IP. Cadastre seu custo como um item consumível; IP agora mede kg livres.",
      )
      return
    }
    if (character.resources[skill.action.resource] < skill.action.cost) {
  ...
  function useInventoryItem(_actionId: string) {
    alert("IP agora mede kg livres. Use um consumível cadastrado na mochila.")
  }

  const characterHazards = activeHazards.filter(
    (hazard: HazardData) =>
      hazard.targetCharacterIds === null ||
      hazard.targetCharacterIds.includes(character.id),
  )
  ...
  async function deleteCustomItem(id: string) {
    ...
    try {
      const { character: updated } = await apiFetch<{ character: Character }>(
        ...
        {
          method: "PATCH",
          body: JSON.stringify({ customItems: newItems }),
        },
      )
      onOptimistic(updated)
    } finally {
  ...
  async function transferItem(
    ...
    itemKind: "equipment" | "custom" = "equipment",
  ) {
    if (!editable && !isGm) return
    const { donor } = await apiFetch<{ donor: Character }>(
      "/api/item-transfers",
      {
        method: "POST",
        body: JSON.stringify({
  ...
  async function handleTransferOwnership() {
    ...
    if (
      !confirm(
        `Deseja transferir o controle permanente desta ficha para este jogador?`,
      )
    )
      return
    ...
    try {
      const { character: updated } = await apiFetch<{ character: Character }>(
        `/api/characters/${character.id}/transfer`,
        {
          method: "POST",
      ...
      alert(
        "Erro ao transferir personagem. Verifique se a rota API de transferência existe.",
      )
    } finally {
      setPending(false)
    }
    ...
    >
            ...
            <AnimatePresence>
              {showLevelUp && (
                ...
                >
                  <motion.div
                    variants={modalVariants}
                    className="rpg-modal relative flex h-full max-h-[85vh] w-full max-w-2xl flex-col border border-primary/50 bg-zinc-950 shadow-2xl"
                    onClick={(event) => event.stopPropagation()}
                  >
                    <div className="flex justify-between items-center p-6 border-b border-border/50 bg-black/40 shrink-0">
                      <div>
                        <h4 className="font-serif text-2xl font-black text-primary flex items-center gap-2">
                          ...
                          {isGm ? "Conceder habilidades" : "Evolução"}
                        </h4>
                        <p className="text-sm text-muted-foreground mt-1">
                          {isGm
                            ? "Concessão gratuita: não consome pontos de evolução nem altera o XP."
                            : `Você tem ${unspentPoints} ponto(s) para investir em Classes.`}
                        </p>
                      </div>
                      <button
                        onClick={() => setShowLevelUp(false)}
                    ...
                    {skillSaveError && (
                      <p
                        role="alert"
                        className="mx-6 mt-4 rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive"
                      >
                        {skillSaveError}
                      </p>
                    )}
                      ...
                      {(() => {
                        ...
                        const filteredClasses = allClasses
                          .map((c) => {
                            ...
                            const availableSkills = c.skills.filter(
                              (s: any) =>
                                skillRank(skillsObj[s.id]) < skillMaxRank(s),
                            )
                          ...
                          .filter((c) => {
                            ...
                            if (classSearch) {
                              ...
                              const matchClass = c.name
                                .toLowerCase()
                                .includes(term)
                              const matchSkill = c.availableSkills.some(
                          ...
                          return (
                            <div className="flex flex-col items-center justify-center py-12 text-center opacity-60">
                              ...
                              <p className="text-muted-foreground font-medium">
                                Nenhuma classe ou habilidade encontrada.
                              </p>
                              {classSearch && (
                    ...
                    <div className="rpg-modal-header flex justify-between items-center p-6 border-b border-border/50 bg-black/40 shrink-0">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          ...
                          vitais em momentos críticos.
                        </p>
                      </div>
                      <button
                        onClick={() => setShowBondsModal(false)}
                        className="rounded-full p-2 bg-white/5 hover:bg-white/10"
                      >
                        <X className="size-5 text-muted-foreground hover:text-white" />
                      </button>
                    </div>
                                ...
                                <div className="flex gap-2 w-full mt-auto">
                                  ...
                                  {editable && (
                                    <Button
                                      ...
                                      className="h-8 w-8 p-0 border-destructive/50 text-destructive hover:bg-destructive/20 shrink-0"
                                    >
            ...
            <AnimatePresence>
              {showInventory && (
                <motion.div
                  ...
                  animate="visible"
                  exit="exit"
                  className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 md:p-8 overflow-hidden"
                  onClick={() => setShowInventory(false)}
                >
                  <motion.div
                    variants={modalVariants}
                    className="rpg-modal relative flex h-full max-h-[85vh] w-full max-w-4xl flex-col overflow-hidden border border-primary/30 bg-zinc-950 shadow-2xl"
                    onClick={(event) => event.stopPropagation()}
                  >
                    <div className="flex justify-between items-center p-6 border-b border-white/10 bg-black/40 shrink-0">
                      <h4 className="font-serif text-2xl md:text-3xl font-black text-[#eee3cf] flex items-center gap-3">
                        <Package className="size-6 text-primary md:size-8" />{" "}
                        Mochila do Herói
                      </h4>
                      <button
                        onClick={() => setShowInventory(false)}
                    ...
                    <div className="flex-1 overflow-y-auto p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] gap-6 custom-scrollbar-sepia">
                      <section className="flex-1 space-y-4">
                        ...
                        <InventoryEquipment
                          ...
                          renderDetail={(text: string) => (
                            <ItemModifiers text={text} />
                          )}
                        />
                        ...
                        {customItems.length > 0 && (
                          <div className="mt-6 pt-6 border-t border-white/10">
                            <h5 className="text-sm font-bold uppercase tracking-widest text-muted-foreground pb-3 flex items-center gap-2">
                              <Sparkles className="size-4" /> Relíquias e
                              Pergaminhos
                            </h5>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              {customItems.map((item: any) => {
                                const isAppBlueprint =
                                  item.type === "app-blueprints"
                                const hasGadgetsSkill =
                                  (character.skills["ti-gadgets"] || 0) > 0
                                const isLockedForThisChar =
                                  isAppBlueprint && !hasGadgetsSkill
                                  ...
                                  <div key={item.id}>
                                    ...
                                    >
                                      <div className="flex items-center gap-3">
                                        ...
                                        {item.type === "app-blueprints" && (
                                          <Package
                                            className={`size-5 ${isLockedForThisChar ? "text-destructive" : "text-blue-500"}`}
                                          />
                                        )}
                                        <div className="flex flex-col">
                                          <span
                                            className={`font-bold text-sm truncate ${isLockedForThisChar ? "text-destructive" : "text-foreground"}`}
                                          >
                                            {item.name}
                                          </span>
                                          {isLockedForThisChar && (
                                            <span className="text-[9px] uppercase tracking-widest text-destructive mt-0.5">
                                              Requer: Inventor
                                            </span>
                                          )}
                                        </div>
                      ...
                      <section className="flex-1 space-y-4">
                        <div className="flex justify-between items-center border-b border-white/10 pb-2">
                          ...
                          <span className="text-sm font-mono font-bold text-primary bg-primary/10 px-2 py-1 rounded">
                            IP: {weightStatus.freePoints}/
                            {displayResources.maxIp} kg
                          </span>
                        </div>
                        <div
                          className={`rounded-xl border p-4 ${inventoryOverflow ? "border-red-500/40 bg-red-500/10" : "border-emerald-500/30 bg-emerald-500/5"}`}
                        >
                          {isGm && (
                            <label className="mb-3 block text-sm">
                              Capacidade da mochila (kg)
                              <input
                                type="number"
                                min="0"
                                step="0.001"
                                defaultValue={inventoryCapacity}
                                key={inventoryCapacity}
                                className="ml-2 w-24 rounded bg-zinc-900 p-2 text-white"
                                onBlur={async (e) => {
                                  const value = Number(e.target.value)
                                  if (value === inventoryCapacity) return
                                  try {
                                    await commitItemUpdate({
                                      resources: { maxIp: value },
                                    })
                                  } catch (error) {
                                    alert(
                                      error instanceof Error
                                    ...
                                    e.target.value = String(inventoryCapacity)
                                  }
                                }}
                              />
                            </label>
                          ...
                          <p className="text-sm">
                            Limite de carga por vigor:{" "}
                            {formatWeight(weightStatus.carryLimitGrams)}
                          </p>
                          <p className="font-bold">
                            {formatWeight(weightStatus.grams)} /{" "}
                            {formatWeight(weightStatus.capacityGrams)} de
                            capacidade
                          </p>
                          <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                            1 ponto = 1 kg. O peso de todos os itens é somado em
                            gramas. A carga de vigor é separada da capacidade da
                            mochila.
                          </p>
                          {inventoryOverflow > 0 && (
                            <p className="mt-3 flex items-center gap-2 text-sm font-bold text-red-300">
                              <AlertTriangle className="size-4" /> Sobrecarga:{" "}
                              {formatWeight(inventoryOverflow)} acima de MIG + {INVENTORY_RULES.migBonusKg}
                              kg; {INVENTORY_RULES.dexPenalty} em testes com DEX.
                            </p>
                          )}
                        </div>
                      </section>
            ...
            <AnimatePresence>
              {viewingItem && (
                <motion.div
                  ...
                  animate="visible"
                  exit="exit"
                  className="fixed inset-0 z-[300] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 overflow-hidden"
                  onClick={() => setViewingItem(null)}
                >
                  <motion.div
                    variants={modalVariants}
                    className={`relative flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden shadow-2xl ${viewingItem.type === "text" ? "rpg-paper text-black border-2 border-[#d4af37]" : "rpg-modal bg-zinc-950 border border-primary/50"}`}
                    onClick={(event) => event.stopPropagation()}
                  >
                    <div
                      className={`flex justify-between items-center p-4 border-b shrink-0 ${viewingItem.type === "text" ? "border-[#d4af37]/30" : "border-white/10 bg-black/40"}`}
                    >
                      <h4
                        className={`font-serif text-xl font-bold flex items-center gap-2 ${viewingItem.type === "text" ? "text-amber-900" : "text-primary"}`}
                      >
                      ...
                      >
                        <X
                          className={`size-5 ${viewingItem.type === "text" ? "text-amber-900" : "text-white"}`}
                        />
                      </button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 custom-scrollbar-sepia">
                      ...
                      {viewingItem.type === "app-blueprints" && (
                        <BlueprintApp
                          character={character}
                          editable={editable}
                          onSpendMp={(cost: number) =>
                            patchResource("mp", -cost)
                          }
                        />
                      )}
                    </div>
                    ...
                    {(editable || isGm) && (
                      <div
                        className={`p-4 border-t shrink-0 flex flex-wrap justify-end gap-2 ${viewingItem.type === "text" ? "border-[#d4af37]/30" : "border-white/10 bg-black/40"}`}
                      >
                        {transferTargets.length > 0 && (
                          <>
                            <select
                              aria-label="Receptor do item"
                              value={customTransferTarget}
                              onChange={(event) =>
                                setCustomTransferTarget(event.target.value)
                              }
                              className="min-w-[180px] rounded-md border border-border/60 bg-background px-3 py-2 text-xs text-foreground"
                            >
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              disabled={!customTransferTarget || pending}
                              onClick={async () => {
                                const index = customItems.findIndex(
                                  (item: any) => item.id === viewingItem.id,
                                )
                                if (index < 0) return
                                await transferItem(
                                  index,
                                  viewingItem.name,
                                  customTransferTarget,
                                  "custom",
                                )
                                setCustomTransferTarget("")
                                setViewingItem(null)
                              }}
                              className="gap-2"
                            >
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => deleteCustomItem(viewingItem.id)}
                          className={`gap-2 ${viewingItem.type === "text" ? "border-red-500/50 text-red-700 hover:bg-red-500/10" : "border-red-500/50 text-red-400 hover:bg-red-500/20"}`}
                        >
            ...
            <AnimatePresence>
              {showTransferModal && editable && (
                ...
                >
                  <motion.div
                    variants={modalVariants}
                    className="rpg-modal relative min-h-[440px] w-full max-w-md border border-primary/50 bg-zinc-950 p-6 py-8 shadow-2xl"
                    onClick={(event) => event.stopPropagation()}
                  >
                    <button
                      onClick={() => setShowTransferModal(false)}
                    ...
                    <p className="text-sm text-muted-foreground mb-6">
                      Selecione para qual jogador você deseja transferir o
                      controle permanente desta ficha.
                    </p>
                    <div className="space-y-2 mb-6">
                      ...
                      <div className="flex flex-col gap-2 max-h-40 overflow-y-auto custom-scrollbar-sepia">
                        {campaignMembers.map((m: any) => (
                          ...
                          >
                            <Shield className="size-4 text-primary" /> {m.name}{" "}
                            {m.role === "gm" && "(Mestre)"}
                          </button>
                        ))}
                        {campaignMembers.length === 0 && (
                          <p className="text-xs text-muted-foreground italic">
                            Nenhum outro jogador conectado.
                          </p>
                        )}
                      </div>
                    </div>
                    <div className="flex justify-end gap-3">
                      ...
                      <Button
                        ...
                        disabled={!transferUserId || pending}
            ...
            <AnimatePresence>
              {showArchiveConfirm && isGm && (
                ...
                >
                  <motion.div
                    variants={modalVariants}
                    className="rpg-modal relative w-full max-w-md border border-destructive/50 bg-zinc-950 p-6 shadow-2xl"
                    onClick={(event) => event.stopPropagation()}
                  >
                    <button
                      onClick={() => setShowArchiveConfirm(false)}
                    ...
                    <div className="p-3 bg-destructive/10 border border-destructive/30 rounded-lg text-xs text-destructive mb-6 leading-relaxed">
                      ...
                      <br />
                      - O jogador perderá o acesso a esta ficha permanentemente.
                      <br />- O personagem se transformará em um{" "}
                      <strong>NPC</strong> e será guardado no seu{" "}
                      <strong>Berçário</strong>.<br />- Você poderá invocá-lo na
                      mesa para combater ou interagir a qualquer momento.
                    </div>
                    <div className="flex justify-end gap-3">
                      <Button
                        variant="ghost"
            ...
            <AnimatePresence>
              {showFrameGallery && (
                <motion.div
                  ...
                  animate="visible"
                  exit="exit"
                  className="fixed inset-0 z-[320] flex items-center justify-center bg-black/85 p-3 backdrop-blur-md sm:p-6"
                  onClick={() => setShowFrameGallery(false)}
                >
                  <motion.div
                    variants={modalVariants}
                    className="rpg-modal max-h-[92vh] w-full max-w-3xl overflow-y-auto border p-6 custom-scrollbar-sepia sm:p-8"
                    onClick={(event) => event.stopPropagation()}
                  >
                    <div className="mb-6 flex items-start justify-between gap-5">
                      <h4 className="rpg-title text-2xl font-bold text-foreground sm:text-3xl">
                        Galeria de Molduras
                      </h4>
                      <button
                        type="button"
                        onClick={() => setShowFrameGallery(false)}
                        className="rounded-sm p-2.5 text-muted-foreground hover:bg-white/10 hover:text-foreground"
                    ...
                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                      {PORTRAIT_FRAMES.map((frame) => {
                        const selected =
                          (character.portraitFrame || "bronze") === frame.id
                        return (
                          <button
                            key={frame.id}
                            type="button"
                            disabled={pending}
                            onClick={() => void savePortraitFrame(frame.id)}
                            className={`portrait-frame-option ${selected ? "is-selected" : ""}`}
                          ...
                          >
                            ...
                            {selected && (
                              <Check className="portrait-frame-check size-5" />
                            )}
                          </button>
                        )
                      })}
              ...
              {showLore && (
                ...
                >
                  ...
                  >
                    ...
                    {editable && (
                      <div className="flex justify-end border-t border-[#8b653d]/30 bg-[#dfc78f] p-4">
                        <Button
                          disabled={savingLore}
                          onClick={saveLore}
                          className="gap-2 bg-[#5d4037] text-[#f5e6c8] hover:bg-[#4e342e]"
                        >
                          <Save className="size-4" />{" "}
                          {savingLore ? "Salvando..." : "Salvar no livro"}
                        </Button>
                      </div>
                    )}
                  </motion.div>
      ...
      <div className="relative z-50 flex w-full flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
          <div className="relative shrink-0">
            ...
            {!isExpanded && editable && (
              <button
                ...
                onClick={(event) => {
                  event.stopPropagation()
                  setShowFrameGallery(true)
                }}
                className="portrait-frame-trigger"
                title="Escolher moldura"
                aria-label="Escolher moldura do retrato"
              >
          ...
          <div
            className="flex-1 min-w-0 flex flex-col items-start cursor-pointer"
            onClick={() => changeExpanded(!isExpanded)}
          >
            <div className="flex justify-between items-start w-full">
              <h3
                className={`font-serif font-bold text-foreground leading-tight truncate transition-[font-size,line-height] duration-[480ms] ease-[cubic-bezier(.4,0,.2,1)] ${isExpanded ? "text-2xl sm:text-[26px] md:text-[28px]" : "text-[20px] sm:text-[22px] md:text-2xl"}`}
              >
                {character.name}{" "}
                <span
                  className={`${isExpanded ? "text-primary text-sm sm:text-base md:text-lg" : "text-zinc-400 text-[12px] sm:text-[13px] md:text-sm"} whitespace-nowrap`}
                >
                  (Nv. {charLevel})
                </span>
              </h3>
            ...
            {!isExpanded ? (
              ...
              <div className="flex flex-col gap-1 mt-1 w-full">
                <div className="font-sans w-full truncate text-[14px] leading-snug text-muted-foreground">
                  {[character.identity, character.origin]
                    .filter(Boolean)
                    .join(" · ") || "Aventureiro"}
                </div>
                <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[13px] font-semibold tabular-nums">
                  <span>
                    <b className="text-[color:var(--hp)]">HP:</b>{" "}
                    <strong className="text-foreground">
                      {displayResources.hp}/{displayResources.maxHp}
                    </strong>
                  </span>
                  <span>
                    <b className="text-[color:var(--mp)]">MP:</b>{" "}
                    <strong className="text-foreground">
                      {displayResources.mp}/{displayResources.maxMp}
                    </strong>
                  </span>
                  <CharacterStatusBadges
                    modifiers={displayModifiers as Modifier[]}
                  />
                </div>
              </div>
            ) : (
              ...
              <>
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  className="mt-1.5 h-8 shrink-0 px-3 text-xs uppercase tracking-widest text-muted-foreground hover:bg-primary/10 hover:text-primary"
                  onClick={(e) => {
                    e.stopPropagation()
                    setShowLore(true)
                  }}
                >
                  <ScrollText className="mr-1.5 size-4 shrink-0" /> Ler História
                  Completa
                </Button>
              </>
            )}
        ...
        <div className="flex w-full shrink-0 flex-col items-stretch gap-2 sm:w-auto sm:items-end">
          <div className="flex items-center justify-between gap-2 sm:justify-start">
            <div className="flex items-center gap-1.5 bg-card px-2 py-1 rounded-full border border-border/60 shadow-sm">
              {editable && (
                <button
                  ...
                  className="flex size-5 items-center justify-center rounded-full border border-border/60 bg-background hover:border-accent hover:bg-accent/10 hover:text-accent transition-colors disabled:opacity-40"
                  aria-label="Diminuir Zenits"
                >
            ...
            >
              <ChevronDown
                className={`size-5 transition-transform duration-300 ${isExpanded ? "rotate-180 text-primary" : ""}`}
              />
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex">
            {editable && (
              <Button
                size="sm"
                variant="outline"
                className="h-9 px-4 text-sm font-semibold border-primary/40 text-primary hover:bg-primary/10 transition-colors"
            ...
            {isGm && onArchive && (
              <Button
                size="sm"
                variant="outline"
                className="rpg-archive-button h-9 px-4 text-sm font-semibold transition-colors"
      ...
      <div
        className="rpg-character-expansion"
        data-open={expansionVisible}
        aria-hidden={!expansionVisible}
      >
        <div className="rpg-character-expansion-clip">
          {(expandedContentMounted || isExpanded) && (
              ...
              <div className="pt-4 mt-2 border-t border-white/10">
                ...
                <div className="rpg-tabs relative z-50 mb-5 grid h-11 w-full grid-cols-3 p-1">
                  <button
                    ...
                    className={`rpg-tab-button flex h-9 min-w-0 flex-1 items-center justify-center rounded-md px-2 text-xs leading-none whitespace-nowrap transition-colors ${sheetTab === "main" ? "is-active font-bold" : "text-muted-foreground"}`}
                  >
                    Principal
                  </button>
                  <button
                    onClick={() => setSheetTab("checks")}
                    className={`rpg-tab-button flex h-9 min-w-0 flex-1 items-center justify-center rounded-md px-2 text-xs leading-none whitespace-nowrap transition-colors ${sheetTab === "checks" ? "is-active font-bold" : "text-muted-foreground"}`}
                  >
                    <span className="truncate">Testes e Perícias</span>
                  </button>
                  <button
                    onClick={() => setSheetTab("modifiers")}
                    className={`rpg-tab-button relative flex h-9 min-w-0 items-center justify-center rounded-md px-7 text-xs leading-none whitespace-nowrap transition-colors ${sheetTab === "modifiers" ? "is-active font-bold" : "text-muted-foreground"}`}
                  >
                    <span className="truncate">Condições</span>
                    {sheetTab !== "modifiers" &&
                      displayModifiers.length > 0 && (
                        <span className="condition-count absolute right-2 flex size-5 items-center justify-center rounded-full text-[10px] font-bold leading-none">
                          {displayModifiers.length}
                        </span>
                      )}
                  </button>
                </div>
                  ...
                  <div className="animate-in fade-in zoom-in-95 duration-200">
                    <div className="rounded-sm border border-border/40 bg-black/35 p-3 shadow-inner sm:p-4">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <p className="text-xs sm:text-sm text-primary font-mono font-bold">
                      ...
                      <div className="rpg-resource-track mb-3 h-1.5 w-full overflow-hidden bg-zinc-800/80 sm:mb-4 sm:h-2">
                        <div
                          className="rpg-resource-fill h-1.5 bg-primary transition-all duration-500 ease-out sm:h-2"
                          style={{
                            width: `${(currentLevelXp / xpRequired) * 100}%`,
                          }}
                        ></div>
                      </div>
                      <div className="flex flex-wrap items-center gap-2">
                        {unspentPoints > 0 || isGm ? (
                          <Button
                            size="sm"
                            variant="default"
                            disabled={!editable && !isGm}
                            className="h-8 text-xs animate-pulse bg-primary/20 text-primary border border-primary/50 hover:bg-primary/30 shrink-0"
                            onClick={() => {
                              setSkillSaveError(null)
                              setShowLevelUp(true)
                            }}
                          >
                            <TrendingUp className="size-3.5 mr-1.5" /> Classes{" "}
                            {unspentPoints > 0
                              ? `(${unspentPoints})`
                              : "(Mestre)"}
                          </Button>
                        ...
                        {isGm && (
                          <div className="flex gap-1.5 ml-auto shrink-0">
                            <Button
                              size="sm"
                              variant="outline"
                              className="h-8 px-2 text-xs border-border/60 bg-background/50 hover:bg-card"
                              onClick={() => patchResource("xp", -1)}
                            >
                              -1
                            </Button>
                            <Button
                    ...
                    <div className="mt-5 grid grid-cols-4 gap-2">
                      {ATTR_KEYS.map((k: AttributeKey) => {
                        const isRolling = rollingAttr === k
                        const canUpgradeAttribute =
                          editable &&
                          unspentAttrPoints > 0 &&
                          character.attributes[k] !== "d20"

                        return (
                          <div key={k} className="relative">
                            <button
                              disabled={!editable || isRolling}
                              onClick={() =>
                                rollDice(k, character.attributes[k])
                              }
                              className={`attribute-roll-button h-20 w-full rounded-sm border text-center shadow-inner ${isRolling ? "is-rolling border-primary bg-primary/20" : editable ? "border-border/60 bg-card/40 hover:border-primary/60 hover:bg-card" : "cursor-default border-border/60 bg-card/40"}`}
                            >
                              {isRolling ? (
                                ...
                                <>
                                  ...
                                  <p className="font-serif text-xl font-black text-primary drop-shadow-sm sm:text-2xl">
                            ...
                            {canUpgradeAttribute && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation()
                                  handleUpgradeAttribute(k)
                                }}
                                className="absolute -top-2 -right-2 flex size-6 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-md hover:scale-110 transition-transform z-10"
                                title={`Evoluir Atributo (${unspentAttrPoints} sobrando)`}
                              >
                    ...
                    <div className="mt-6 flex flex-col gap-4">
                      <div className="flex items-center gap-3">
                        <div className="flex-1">
                          <ResourceBar
                            ...
                            short="HP"
                            icon={<Heart className="size-4" />}
                            current={displayResources.hp}
                            max={displayResources.maxHp}
                            colorVar="--hp"
                            editable={editable}
                        ...
                        {isGm && (
                          <div className="flex flex-col gap-1 shrink-0">
                            <button
                              onClick={() => patchResource("maxHp", 1)}
                              className="flex items-center justify-center size-5 bg-background border border-border/60 rounded hover:border-[color:var(--hp)] hover:text-[color:var(--hp)] transition-colors"
                              title="Aumentar Vida Máxima Manualmente"
                            >
                              +
                            </button>
                            <button
                              onClick={() => patchResource("maxHp", -1)}
                              className="flex items-center justify-center size-5 bg-background border border-border/60 rounded hover:border-[color:var(--hp)] hover:text-[color:var(--hp)] transition-colors"
                              title="Diminuir Vida Máxima Manualmente"
                            >
                              -
                            </button>
                          </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex-1">
                          <ResourceBar
                            ...
                            short="MP"
                            icon={<WandSparkles className="size-4" />}
                            current={displayResources.mp}
                            max={displayResources.maxMp}
                            colorVar="--mp"
                            editable={editable}
                        ...
                        {isGm && (
                          <div className="flex flex-col gap-1 shrink-0">
                            <button
                              onClick={() => patchResource("maxMp", 1)}
                              className="flex items-center justify-center size-5 bg-background border border-border/60 rounded hover:border-[color:var(--mp)] hover:text-[color:var(--mp)] transition-colors"
                              title="Aumentar Mana Máxima Manualmente"
                            >
                              +
                            </button>
                            <button
                              onClick={() => patchResource("maxMp", -1)}
                              className="flex items-center justify-center size-5 bg-background border border-border/60 rounded hover:border-[color:var(--mp)] hover:text-[color:var(--mp)] transition-colors"
                              title="Diminuir Mana Máxima Manualmente"
                            >
                              -
                            </button>
                          </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex-1">
                          <ResourceBar
                            label="Mochila (kg livres)"
                            short="IP"
                            icon={<Backpack className="size-4" />}
                            current={weightStatus.freePoints}
                            max={inventoryCapacity}
                            colorVar="--ip"
                            editable={false}
                            onChange={() => {}}
                          />
                        ...
                        {isGm && (
                          <div className="flex flex-col gap-1 shrink-0">
                            <button
                              onClick={() => patchResource("maxIp", 1)}
                              className="flex items-center justify-center size-5 bg-background border border-border/60 rounded hover:border-[color:var(--ip)] hover:text-[color:var(--ip)] transition-colors"
                              title="Aumentar IP Máximo Manualmente"
                            >
                              +
                            </button>
                            <button
                              onClick={() => patchResource("maxIp", -1)}
                              className="flex items-center justify-center size-5 bg-background border border-border/60 rounded hover:border-[color:var(--ip)] hover:text-[color:var(--ip)] transition-colors"
                              title="Diminuir IP Máximo Manualmente"
                            >
                              -
                            </button>
                          </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-5">
                      ...
                      <div className="flex-1 min-w-[140px] flex items-center justify-between rounded-lg border border-[color:var(--fp)]/30 bg-[color:var(--fp)]/5 px-3 py-2">
                        <span className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-[color:var(--fp)]">
                          <Sparkles className="size-3.5" /> Inspiração
                        </span>
                        <div className="flex items-center gap-1.5 shrink-0">
                          {editable && (
                            <button
                              onClick={() => patchResource("fp", -1)}
                              className="flex size-5 items-center justify-center rounded border border-border/60 bg-background hover:border-[color:var(--fp)] hover:bg-[color:var(--fp)]/10 transition-colors"
                            >
                              <Minus className="size-3" />
                            </button>
                          )}
                          <span className="w-5 text-center font-mono text-sm font-bold text-[color:var(--fp)]">
                            {displayResources.fp}
                          </span>
                          {editable && (
                            <button
                      ...
                      <div className="w-full flex gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          className="flex-1 h-auto py-2.5 border-primary/30 bg-primary/5 text-primary hover:bg-primary/20 hover:border-primary/50 transition-colors"
                          onClick={() => setShowInventory(true)}
                        >
                          <Package className="size-4 mr-2 shrink-0" />{" "}
                          <span className="text-[#eee3cf]">Mochila</span>
                        </Button>

                        <Button
                          variant="outline"
                          size="sm"
                          className="flex-1 h-auto py-2.5 border-accent/30 bg-accent/5 text-accent hover:bg-accent/20 hover:border-accent/50 transition-colors"
                          onClick={() => setShowStore(true)}
                        >
                          <Store className="size-4 mr-2 shrink-0" />{" "}
                          <span className="text-[#eee3cf]">Loja</span>
                        </Button>
                        ...
                        >
                          ...
                          <span className="text-[#eee3cf]">Laços</span>
                        </Button>
                      </div>
                    </div>
                    ...
                    <div className="mt-8 pt-5 border-t border-border/30">
                      <div className="mb-4 flex items-center justify-between gap-3">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {learnedSkillGroups.length} classe(s)
                        </span>
                      </div>
                      <div className="space-y-4">
                        {learnedSkillGroups.map((group: any) => (
                          <section
                            key={group.id}
                            className="overflow-hidden rounded-xl border border-primary/25 bg-card/30 shadow-sm"
                          >
                            <header className="flex flex-wrap items-center justify-between gap-3 border-b border-primary/15 bg-gradient-to-r from-primary/15 to-transparent px-4 py-3">
                              <div className="flex min-w-0 items-center gap-3">
                                ...
                                <div className="min-w-0">
                                  <h4 className="break-words font-serif text-base font-bold text-primary">
                                    {group.name}
                                  </h4>
                                  <p className="text-xs text-muted-foreground">
                                    {group.learnedSkills.length} habilidade(s)
                                    aprendida(s)
                                  </p>
                                </div>
                              </div>
                            ...
                            <div className="space-y-2 p-3">
                              {group.learnedSkills.map((skill: any) => {
                                ...
                                const freeRanks = getFreeSkillRanks(
                                  skill.id,
                                  lvl,
                                  allMods,
                                  customSkillIds,
                                ...
                                const skillExpanded =
                                  expandedSkillId === skillKey
                                return (
                                  <div
                                    key={skill.id}
                                    className={`overflow-hidden rounded-lg border transition-colors ${skillExpanded ? "border-primary/40 bg-primary/5" : "border-border/40 bg-background/30 hover:border-primary/25"}`}
                                  >
                                    <button
                                      type="button"
                                    ...
                                    >
                                      <div className="min-w-0">
                                        ...
                                        <span className="mt-1 block text-[11px] text-muted-foreground">
                                          {group.name}
                                          {freeRanks > 0
                                            ? ` · ${freeRanks} nível(is) grátis`
                                            : ""}
                                        </span>
                                      </div>
                                      <div className="flex shrink-0 items-center gap-2">
                                        <span className="rounded-md bg-primary/10 px-2 py-1 font-mono text-[10px] text-primary">
                                          Nv. {lvl}/{skillMaxRank(skill)}
                                        </span>
                                        <ChevronDown
                                          className={`size-4 text-muted-foreground transition-transform ${skillExpanded ? "rotate-180" : ""}`}
                                        />
                                      </div>
                                    </button>
                                    {skillExpanded && (
                        ...
                        {unresolvedSkills.length > 0 && (
                          <section className="rounded-xl border border-border/50 bg-card/30 p-4">
                            <h4 className="text-sm font-bold">
                              Classe não disponível no catálogo
                            </h4>
                            <p className="mt-1 text-xs text-muted-foreground">
                              Os níveis estão preservados. Carregue a classe
                              para ver os nomes e descrições.
                            </p>
                            {unresolvedSkills.map(([id, level]) => (
                              <p
                                key={id}
                                className="mt-2 break-all text-xs text-muted-foreground"
                              >
                                {id} · Nv. {skillRank(level)}
                              </p>
                            ))}
                          </section>
                        )}
                  ...
                  <div className="flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
                    <div className="flex justify-between items-center bg-black/40 p-3 rounded-xl border border-white/10 shadow-inner">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          Efeitos, buffs e debuffs ativos.
                        </p>
                      </div>
                      {isGm && (
                        <Button
                    ...
                    <div className="flex flex-col gap-2">
                      {displayModifiers?.map((mod: any) => (
                        <div
                          key={mod.id}
                          className="p-3 bg-card/40 border border-border/50 rounded-lg"
                        >
                          {editingModId === mod.id && isGm ? (
                            <div className="flex flex-col gap-2">
                              <input
                                className="bg-black/50 border border-white/10 rounded-md px-3 py-2 text-sm text-foreground focus:outline-none focus:border-primary/50"
                                value={modDraft.name}
                                onChange={(e) =>
                                  setModDraft({
                                    ...modDraft,
                              ...
                              <div className="flex gap-2">
                                ...
                                <select
                                  className="bg-black/50 border border-white/10 rounded-md px-3 py-2 text-sm text-foreground w-1/2 focus:outline-none focus:border-primary/50"
                                  value={modDraft.target}
                                  onChange={(e) =>
                                    setModDraft({
                                      ...modDraft,
                                      target: e.target.value,
                                ...
                                >
                                  <option value="all">
                                    Todos os Testes Gerais
                                  </option>
                                  <optgroup label="Por Atributo Envolvido">
                                    <option value="mig">
                                      Qualquer rolagem usando Vigor (MIG)
                                    </option>
                                    <option value="dex">
                                      Qualquer rolagem usando Destreza (DEX)
                                    </option>
                                    <option value="ins">
                                      Qualquer rolagem usando Intuição (INS)
                                    </option>
                                    <option value="wlp">
                                      Qualquer rolagem usando Vontade (WLP)
                                    </option>
                                  </optgroup>
                              ...
                              <div className="flex justify-end gap-2 mt-2">
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    const newMods = (
                                      character as any
                                    ).customModifiers.map((m: any) =>
                                      m.id === mod.id ? modDraft : m,
                                    )
                            ...
                            <div className="flex justify-between items-center gap-2">
                              <div>
                                ...
                                <p className="text-[10px] text-muted-foreground uppercase tracking-widest mt-1">
                                  Alvo: {mod.target}{" "}
                                  <span className="mx-1">•</span> Bônus:{" "}
                                  {mod.value > 0 ? `+${mod.value}` : mod.value}
                                </p>
                              </div>
                              <div className="flex gap-2 items-center shrink-0">
                                {!isGm &&
                                  ((displayResources as any).lp || 0) > 0 &&
                                  editable && (
                                    <Button
                                      size="sm"
                                      variant="outline"
                                      ...
                                      onClick={() => {
                                        if (
                                          confirm(
                                            `Gastar 1 Ponto de Lore (LP) para melhorar a condição "${mod.name}" em +1?`,
                                          )
                                        ) {
                                          patchResource("lp", -1)
                                          const newMods = (
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            <div className="flex justify-end gap-2 mt-2">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  const newMods = [
                                    ...((character as any).customModifiers ||
                                      []),
                                    modDraft,
                                  ]
                                  updateModifiers(newMods)
                ...
                {isGm && onKill && displayResources.hp <= 0 && (
                  <div className="mt-6 pt-5 border-t border-destructive/50">
                    ...
                    >
                      <Skull className="size-5" /> Confirmar Morte e Recolher
                      Loots
                    </Button>
                  </div>
                )}
